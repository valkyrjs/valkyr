...
import { type AuthDB, schema } from "./database.ts";
 ...
 * application.
 */
export class SQLiteAuth<TPermissions extends Permissions> implements Repository<TPermissions> {
  ...
  async addRole(payload: RolePayload<TPermissions>): Promise<Role<TPermissions>> {
    const roleId = await this.roles.addRole(payload);
    return new Role({ roleId, ...payload }, this);
  }
